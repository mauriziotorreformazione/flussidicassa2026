...
def _get_pdc_from_codice(codice: str, tipo: str) -> str:
    ...
    if not codice:
        return ""
    cod = codice.strip().upper().replace("/", "").replace(".", "").replace("-", "")
    # Normalizza: "E01/01" -> "E01", "A01" -> "A01"
    prefix = cod[:3]
    if tipo in ("entrata_comp", "residuo_attivo"):
        return _MAP_ENTRATE_PDC.get(prefix, "E.3.05")
    else:
        return _MAP_SPESE_PDC.get(prefix, "U.1.10")
